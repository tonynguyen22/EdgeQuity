import { FUNDRA_COLUMNS, formatFundraValue, getColumnValue } from '../metrics';
import type { FundraColumn, FundraMetricGroup, FundraStockRecord } from '../types';

interface StockDetailProps {
  stock: FundraStockRecord;
  onBack: () => void;
}

interface MetricGroupDefinition {
  id: Exclude<FundraMetricGroup, 'profile'>;
  label: string;
}

const METRIC_GROUPS: MetricGroupDefinition[] = [
  { id: 'valuation', label: 'Valuation' },
  { id: 'profitability', label: 'Profitability' },
  { id: 'growth', label: 'Growth' },
  { id: 'financialHealth', label: 'Financial health' },
  { id: 'cashFlow', label: 'Cash flow' },
  { id: 'dividends', label: 'Dividends' },
];

const HISTORY_MONEY_FIELDS = [
  { id: 'revenue', label: 'Revenue' },
  { id: 'grossProfit', label: 'Gross Profit' },
  { id: 'operatingIncome', label: 'Operating Income' },
  { id: 'netIncome', label: 'Net Income' },
  { id: 'freeCashFlow', label: 'Free Cash Flow' },
  { id: 'totalDebt', label: 'Total Debt' },
  { id: 'totalEquity', label: 'Equity' },
] as const;

type HistoryMoneyField = (typeof HISTORY_MONEY_FIELDS)[number]['id'];

function getGroupColumns(group: FundraMetricGroup): FundraColumn[] {
  return FUNDRA_COLUMNS.filter((column) => column.group === group);
}

function formatHistoryMoney(value: number | null): string {
  return formatFundraValue(value, 'money');
}

export default function StockDetail({ stock, onBack }: StockDetailProps) {
  const sectorLine = [stock.sector, stock.industry].filter(Boolean).join(' / ') || 'Classification unavailable';

  return (
    <div className="space-y-4">
      <button
        type="button"
        className="rounded-md px-3 py-2 text-sm font-semibold transition-colors hover:bg-[var(--vw-bg-hover)]"
        style={{
          background: 'var(--vw-bg-surface)',
          border: '1px solid var(--vw-border-lit)',
          color: 'var(--vw-text-secondary)',
        }}
        onClick={onBack}
      >
        Back to screener
      </button>

      <header
        className="border-b pb-4"
        style={{ borderColor: 'var(--vw-border)' }}
      >
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="font-mono text-sm font-semibold" style={{ color: 'var(--vw-accent)' }}>
              {stock.ticker}
            </p>
            <h2 className="mt-1 text-2xl font-semibold tracking-normal">{stock.name}</h2>
            <p className="mt-1 text-sm" style={{ color: 'var(--vw-text-secondary)' }}>
              {sectorLine}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm md:text-right">
            <HeaderMetric label="Price" value={`${stock.currency ?? 'USD'} ${formatFundraValue(stock.price, 'number')}`} />
            <HeaderMetric label="Market Cap" value={formatFundraValue(stock.marketCap, 'money')} />
            <HeaderMetric label="Enterprise Value" value={formatFundraValue(stock.enterpriseValue, 'money')} />
            <HeaderMetric label="FCF Yield" value={formatFundraValue(stock.valuation.fcfYield, 'percent')} />
          </div>
        </div>
      </header>

      {stock.warnings.length > 0 && (
        <section className="vw-card p-4">
          <h3 className="text-sm font-semibold uppercase" style={{ color: 'var(--vw-text-tertiary)' }}>
            Data notes
          </h3>
          <ul className="mt-3 list-disc space-y-1 pl-5 text-sm" style={{ color: 'var(--vw-text-secondary)' }}>
            {stock.warnings.map((warning) => (
              <li key={warning}>{warning}</li>
            ))}
          </ul>
        </section>
      )}

      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {METRIC_GROUPS.map((group) => (
          <MetricGroupCard key={group.id} stock={stock} group={group} />
        ))}
      </section>

      <section className="vw-card overflow-hidden">
        <div
          className="border-b px-4 py-3"
          style={{ borderColor: 'var(--vw-border)' }}
        >
          <h3 className="text-sm font-semibold uppercase" style={{ color: 'var(--vw-text-tertiary)' }}>
            Historical fundamentals
          </h3>
        </div>
        <div className="overflow-x-auto">
          <table className="min-w-[980px] table-fixed border-collapse text-sm">
            <thead style={{ background: 'var(--vw-bg-raised)', color: 'var(--vw-text-tertiary)' }}>
              <tr className="border-b" style={{ borderColor: 'var(--vw-border)' }}>
                <th scope="col" className="w-[92px] px-4 py-2 text-left text-xs font-semibold uppercase">
                  Year
                </th>
                {HISTORY_MONEY_FIELDS.map((field) => (
                  <th key={field.id} scope="col" className="w-[126px] px-3 py-2 text-right text-xs font-semibold uppercase">
                    {field.label}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y" style={{ borderColor: 'var(--vw-border-dim)' }}>
              {stock.history.map((year) => (
                <tr key={year.year}>
                  <th scope="row" className="px-4 py-2 text-left font-mono text-sm font-semibold">
                    {year.year}
                  </th>
                  {HISTORY_MONEY_FIELDS.map((field) => (
                    <td key={field.id} className="px-3 py-2 text-right font-mono tabular-nums">
                      {formatHistoryMoney(year[field.id as HistoryMoneyField])}
                    </td>
                  ))}
                </tr>
              ))}
              {stock.history.length === 0 && (
                <tr>
                  <td
                    colSpan={HISTORY_MONEY_FIELDS.length + 1}
                    className="px-4 py-8 text-center text-sm"
                    style={{ color: 'var(--vw-text-tertiary)' }}
                  >
                    No historical fundamentals available
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
      </section>
    </div>
  );
}

function HeaderMetric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-xs font-semibold uppercase" style={{ color: 'var(--vw-text-tertiary)' }}>
        {label}
      </p>
      <p className="font-mono tabular-nums" style={{ color: 'var(--vw-text-primary)' }}>
        {value}
      </p>
    </div>
  );
}

function MetricGroupCard({ stock, group }: { stock: FundraStockRecord; group: MetricGroupDefinition }) {
  const columns = getGroupColumns(group.id);

  return (
    <article className="vw-card p-4">
      <h3 className="text-sm font-semibold uppercase" style={{ color: 'var(--vw-text-tertiary)' }}>
        {group.label}
      </h3>
      <dl className="mt-3 divide-y" style={{ borderColor: 'var(--vw-border-dim)' }}>
        {columns.map((column) => {
          const formattedValue = formatFundraValue(getColumnValue(stock, column), column.format);
          const isMissing = formattedValue === '-';

          return (
            <div key={column.id} className="flex items-baseline justify-between gap-4 py-2">
              <dt className="min-w-0 truncate text-sm" style={{ color: 'var(--vw-text-secondary)' }}>
                {column.label}
              </dt>
              <dd
                className="shrink-0 font-mono text-sm tabular-nums"
                style={{ color: isMissing ? 'var(--vw-text-tertiary)' : 'var(--vw-text-primary)' }}
              >
                {formattedValue}
              </dd>
            </div>
          );
        })}
      </dl>
    </article>
  );
}
