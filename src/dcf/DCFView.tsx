...
import { Search, TrendingUp, TrendingDown, AlertCircle, Target, ArrowLeft, Activity, DollarSign, BarChart3, FileSpreadsheet } from 'lucide-react';
...
import type { DCFInputs, DCFResult, FormatUnit, ScenarioComparison, BridgeItem, ScenarioType } from './types';
...
export default function DCFView() {
  ...
  // ── Data fetching ──────────────────────────────────────────────────────────
  const { data, loading, error, analystTarget, reset } = useDCFData(ticker);
  ...
  const dcfState = useMemo(() => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Unable to build DCF model for this ticker.';
      return { dcf: null, calcError: message };
    }
  }, [data, deferredInputs]);
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    const input = tickerInput.trim().toUpperCase();
  ...
  const handleLegendClick = (e: { dataKey: string }, chartKeys: string[]) => {
    setUiState(prev => {
  ...
  const bridgeData = useMemo((): BridgeItem[] | null => {
    if (!dcf) return null;
    const pvFcff = dcf.projections.reduce((s: number, p: { discountedFcff: number }) => s + p.discountedFcff, 0);
  ...
  const handlePrint = () => {
    ...
    printDCF({
      dcf,
      ticker,
      profileName: data.profile?.name ?? ticker,
      activeScenario,
      inputs: dcfInputs,
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-8 space-y-6">
                  ...
                  {dcf.historicalSummary.length >= 2 && (() => {
                    const histChart = dcf.historicalSummary.map((h: { year: string; rev: number; revGrowth: number; grossMargin: number; ebitMargin: number; ebitdaMargin: number; netProfitMargin: number }) => ({
                          ...
                          return (
                            <tr key={ri} className="border-t border-slate-700/30">
                              ...
                              {row.map((iv: number | null, ci: number) => {
                                ...
                                const pctVal = iv !== null && dcf.currentPrice > 0 ? (iv - dcf.currentPrice) / dcf.currentPrice : null;
                                const aboveAnalyst = iv !== null && analystTarget && iv >= analystTarget.mean;
                                const bg = iv === null ? '' : pctVal !== null && pctVal >= 0.10 ? 'bg-emerald-500/25' : pctVal !== null && pctVal >= 0 ? 'bg-emerald-500/10' : pctVal !== null && pctVal >= -0.10 ? 'bg-red-500/10' : 'bg-red-500/25';
                                const textColor = iv === null ? 'text-slate-500' : pctVal !== null && pctVal >= 0 ? 'text-emerald-400' : 'text-red-400';
                                return (
                ...
                <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6">
                  ...
                  <div className="mt-5 space-y-2">
                    {(['bear', 'base', 'bull'] as const).map(key => {
                      ...
                      const pctBar = Math.min(s.price / maxPrice * 100, 100);
                      ...
                      return (
                        <div key={key} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 relative h-4 bg-slate-700/40 rounded overflow-visible">
                            <div className={`absolute inset-y-0 left-0 ${colors[key]} rounded opacity-70`} style={{ width: `${pctBar}%` }} />
