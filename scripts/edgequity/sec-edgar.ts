...
function isFiscalQuarterDuration(days: number | null): boolean {
  return days !== null && days >= 70 && days <= 120;
}
...
export function pickBetterQuarterFact<T extends SecQuarterlyUsdRow>(left: T, right: T): T {
  const leftDays = quarterDurationDays(left);
  const rightDays = quarterDurationDays(right);
  const leftIsQuarter = isFiscalQuarterDuration(leftDays);
  const rightIsQuarter = isFiscalQuarterDuration(rightDays);

  if (leftIsQuarter && !rightIsQuarter) return left;
  if (rightIsQuarter && !leftIsQuarter) return right;
